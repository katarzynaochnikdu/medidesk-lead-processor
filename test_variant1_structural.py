"""
Test Wariant 1: Strukturalny
Dane mapowane do osobnych pol (first_name, last_name, email, phone, company).
AI dostaje strukturalne pola i moze je nadpisac/poprawic.
"""
...
async def main():
    """Glowna funkcja testowa - wariant strukturalny."""
    print("=" * 80)
    print("TEST WARIANT 1: STRUKTURALNY")
    print("=" * 80)
    ...
    # Inicjalizuj serwis (prawdziwe polaczenia z Zoho + GUS)
    ...
    print(f"\n==> Przetwarzam {len(df_input)} leadow (STRUKTURALNY)...\n")
    
    for idx, row in df_input.iterrows():
        # Mapuj kolumny z Excel na pola API - STRUKTURALNIE
        raw_phone = row.get("Telefon komorkowy")
        if pd.isna(raw_phone):
            raw_phone = row.get("Telefon komórkowy")
        # Konwertuj telefon na string (Excel moze go czytac jako int)
        if pd.notna(raw_phone):
            raw_phone = str(int(raw_phone)) if isinstance(raw_phone, (int, float)) else str(raw_phone)
        else:
            raw_phone = None
        
        # WARIANT STRUKTURALNY - osobne pola
        lead_data = {
            ...
            "description": row.get("Tresc wiadomosci z formularza") if pd.notna(row.get("Tresc wiadomosci z formularza")) else row.get("Treść wiadomości z formularza"),
        }
        # Usun NaN dla pozostalych
        lead_data = {
        ...
        try:
            # Przetwórz - PELNY PIPELINE z AI
            output = await service.process_lead(
                raw_data=lead_data,
                skip_ai=False,  # Z normalizacja AI (Vertex AI/Gemini)
                skip_gus=False,  # Z GUS - wyszukiwanie po NIP
                skip_duplicates=False,  # Z deduplikacja w Zoho CRM
            )
            ...
            # Sprawdz czy NIP byl znaleziony przez Brave
            nip_source = ""
            if normalized.nip:
            ...
            result = {
                ...
                # Oryginalne dane (input)
                "input_raw_name": lead_data.get("raw_name", ""),
    ...
    # Zapisz wyniki
    output_file = "test_results_variant1.xlsx"
    ...
    print("=" * 80)
    print("PODSUMOWANIE WARIANT 1 (STRUKTURALNY)")
    print("=" * 80)
